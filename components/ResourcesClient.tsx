...
const guides = [
  ...
  { slug: "what-is-phishing", icon: ShieldCheckIcon, tag: "Cybersecurity", title: "What Is Phishing and How Attacks Actually Reach Your Inbox", text: "Practical guidance for reducing risk and responding with confidence.", time: "6 min read", tone: "bg-mission-ink" },
  { slug: "mfa-app-text-key", icon: ShieldCheckIcon, tag: "Cybersecurity", title: "MFA Explained: App vs. Text vs. Hardware Key", text: "Practical guidance for reducing risk and responding with confidence.", time: "6 min read", tone: "bg-mission-ink" },
  { slug: "first-hour-ransomware", icon: ShieldCheckIcon, tag: "Cybersecurity", title: "What to Do in the First Hour of a Ransomware Attack", text: "Practical guidance for reducing risk and responding with confidence.", time: "6 min read", tone: "bg-mission-ink" },
  { slug: "cyber-insurance-questionnaires", icon: ShieldCheckIcon, tag: "Cybersecurity", title: "Cyber Insurance Questionnaires: What the Questions Really Mean", text: "Practical guidance for reducing risk and responding with confidence.", time: "6 min read", tone: "bg-mission-ink" },
  { slug: "password-managers-small-teams", icon: ShieldCheckIcon, tag: "Cybersecurity", title: "Password Managers for Small Teams", text: "Practical guidance for reducing risk and responding with confidence.", time: "6 min read", tone: "bg-mission-ink" },
  { slug: "why-antivirus-isnt-enough", icon: ShieldCheckIcon, tag: "Cybersecurity", title: "Why Antivirus Alone Isn't Enough Anymore", text: "Practical guidance for reducing risk and responding with confidence.", time: "6 min read", tone: "bg-mission-ink" },
  { slug: "business-email-compromise", icon: ShieldCheckIcon, tag: "Cybersecurity", title: "Business Email Compromise: How Wire Fraud Happens", text: "Practical guidance for reducing risk and responding with confidence.", time: "6 min read", tone: "bg-mission-ink" },
  { slug: "security-awareness-training", icon: ShieldCheckIcon, tag: "Cybersecurity", title: "Employee Security Awareness Training: What Actually Reduces Incidents", text: "Practical guidance for reducing risk and responding with confidence.", time: "6 min read", tone: "bg-mission-ink" },
  { slug: "dark-web-monitoring", icon: ShieldCheckIcon, tag: "Cybersecurity", title: "Dark Web Monitoring: What It Can and Can't Tell You", text: "Practical guidance for reducing risk and responding with confidence.", time: "6 min read", tone: "bg-mission-ink" },
  { slug: "backed-up-or-synced", icon: ShieldCheckIcon, tag: "Cybersecurity", title: "Is Your Data Backed Up or Just Synced?", text: "Practical guidance for reducing risk and responding with confidence.", time: "6 min read", tone: "bg-mission-ink" },
  { slug: "three-two-one-backup", icon: ShieldCheckIcon, tag: "Cybersecurity", title: "The 3-2-1 Backup Rule", text: "Practical guidance for reducing risk and responding with confidence.", time: "6 min read", tone: "bg-mission-ink" },
  { slug: "test-backup-restore", icon: ShieldCheckIcon, tag: "Cybersecurity", title: "How to Test a Backup Restore", text: "Practical guidance for reducing risk and responding with confidence.", time: "6 min read", tone: "bg-mission-ink" },
];
...
export default function ResourcesPage() {
  ...
  const filteredGuides = useMemo(() => guides.filter((guide) => {
    ...
    return topicMatch && `${guide.title} ${guide.text} ${guide.tag}`.toLowerCase().includes(query.toLowerCase());
  }), [activeTopic, query]);
  return (
    <main>
      <SiteHeader />
      ...
      <section id="guides" className="bg-mission-mist py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm font-extrabold uppercase tracking-[0.18em] text-mission-navy">The knowledge library</p><h2 className="mt-3 text-4xl font-black tracking-tight text-mission-ink sm:text-5xl">Start with what is on your desk today.</h2></div><label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-500"><MagnifyingGlassIcon className="h-5 w-5 text-mission-navy" /><span className="sr-only">Search guides</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search guides" className="w-44 bg-transparent outline-none placeholder:text-slate-400" /></label></div>
          <div className="mt-10 flex flex-wrap gap-2">{topics.map(([topic], i) => <button type="button" key={topic} onClick={() => setActiveTopic(topic)} className={`rounded-full px-4 py-2 text-sm font-bold transition hover:-translate-y-0.5 hover:shadow-sm ${activeTopic === topic || (i === 0 && activeTopic === "All guides") ? "bg-mission-navy text-white" : "border border-slate-200 bg-white text-slate-600 hover:border-mission-gold"}`}>{topic}</button>)}</div>
          <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filteredGuides.map(({ slug, icon: Icon, tag, title, text, time, tone }) => <article key={title} className="group flex min-h-[310px] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><div className={`flex h-28 items-center justify-between px-7 ${tone}`}><Icon className={`h-10 w-10 ${tone === "bg-mission-gold" ? "text-mission-ink" : "text-mission-gold"}`} /><span className={`rounded-full px-3 py-1 text-xs font-black uppercase tracking-[0.15em] ${tone === "bg-mission-gold" ? "bg-mission-ink/10 text-mission-ink" : "bg-white/10 text-white"}`}>{time}</span></div><div className="flex flex-1 flex-col p-7"><p className="text-xs font-black uppercase tracking-[0.16em] text-mission-navy">{tag}</p><h3 className="mt-3 text-2xl font-black tracking-tight text-mission-ink">{title}</h3><p className="mt-3 flex-1 leading-7 text-slate-600">{text}</p><Link href={`/resources/${slug}`} className="mt-6 inline-flex items-center gap-2 font-extrabold text-mission-navy transition group-hover:text-mission-gold">Read article <ArrowRightIcon className="h-4 w-4" /></Link></div></article>)}{filteredGuides.length === 0 && <p className="col-span-full rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center font-bold text-slate-600">No guides match that search yet. Try another phrase.</p>}</div>
