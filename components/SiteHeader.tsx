    ...
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 backdrop-blur">
      ...
      <div className="relative mx-auto flex max-w-7xl items-center justify-between bg-mission-navy px-6 py-4 lg:bg-transparent lg:px-8">
        <Link href="/" aria-label="Mission Technology Solutions home" className="shrink-0" onClick={() => setMenuOpen(false)}>
          <Image src="/brand/mission-wordmark-white.png" alt="Mission Technology Solutions" width={4980} height={539} priority className="absolute left-1/2 top-1/2 h-auto w-[255px] -translate-x-1/2 -translate-y-1/2 sm:static sm:w-[260px] sm:translate-x-0 sm:translate-y-0 lg:hidden" />
          <Image src="/brand/mission-wordmark-blue.png" alt="Mission Technology Solutions" width={4980} height={539} priority className="hidden h-auto w-[260px] lg:block xl:w-[300px]" />
        </Link>
          ...
          {nav.map((item) => {
            ...
            return hasDropdown ? (
              <div key={item.href} className="relative" onMouseEnter={() => setOpenDropdown(item.label)} onMouseLeave={() => setOpenDropdown(null)}>
                ...
                {openDropdown === item.label && <div className="absolute left-1/2 top-full z-50 w-[min(94vw,860px)] -translate-x-1/2 pt-4" onMouseLeave={() => setOpenDropdown(null)}><div className="grid overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-3 shadow-2xl shadow-mission-ink/25 sm:grid-cols-[0.82fr_1.18fr]"><div className="relative overflow-hidden rounded-[1.5rem] bg-mission-navy p-6 text-white"><div className="absolute -right-12 -top-12 h-36 w-36 rounded-full border-[18px] border-mission-gold/20" /><div className="absolute -bottom-16 -left-10 h-40 w-40 rounded-full border-[18px] border-white/10" /><div className="relative flex h-full flex-col"><span className="text-[0.68rem] font-black uppercase tracking-[0.22em] text-mission-gold">Mission {item.label === "Services" ? "in motion" : "by your side"}</span><h3 className="mt-5 max-w-[13rem] text-2xl font-black leading-tight">{item.label === "Services" ? "Build a stronger technology foundation." : "Built around the way your team works."}</h3><p className="mt-4 max-w-[15rem] text-sm leading-6 text-slate-200">{item.label === "Services" ? "Practical expertise across the systems your organization depends on every day." : "Industry-aware guidance, support, and security for the work that matters."}</p><Link href={item.href} className="mt-auto inline-flex items-center gap-2 pt-8 text-sm font-black text-mission-gold" onClick={() => setOpenDropdown(null)}>Explore all {item.label.toLowerCase()} <span aria-hidden="true">↗</span></Link></div></div><div className="p-2 sm:p-4"><div className="mb-3 flex items-center justify-between px-2"><span className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Explore {item.label}</span><span className="h-px w-12 bg-mission-gold/60" /></div><div className="grid gap-1 sm:grid-cols-2">{links.map(([label, description, href]) => <Link key={label} href={href ?? `/industries#${label.toLowerCase().replaceAll(" ", "-")}`} className="group flex gap-3 rounded-2xl px-3 py-3.5 transition hover:bg-mission-mist" onClick={() => setOpenDropdown(null)}><span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-mission-gold transition group-hover:scale-125 group-hover:shadow-[0_0_0_5px_rgba(211,166,59,0.15)]" /><span><span className="block text-sm font-extrabold text-mission-ink">{label}</span><span className="mt-0.5 block text-xs leading-5 text-slate-500">{description}</span></span></Link>)}</div></div></div></div>}
              </div>
