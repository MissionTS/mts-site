    ...
    <main>
      ...
      <section id="guides" className="bg-mission-mist py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm font-extrabold uppercase tracking-[0.18em] text-mission-navy">The knowledge library</p><h2 className="mt-3 text-4xl font-black tracking-tight text-mission-ink sm:text-5xl">Start with what is on your desk today.</h2></div><label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-500"><MagnifyingGlassIcon className="h-5 w-5 text-mission-navy" /><span className="sr-only">Search guides</span><input placeholder="Search guides" className="w-44 bg-transparent outline-none placeholder:text-slate-400" /></label></div>
          <div className="mt-10 flex flex-wrap gap-2">{topics.map(([topic], i) => <button type="button" key={topic} className={`rounded-full px-4 py-2 text-sm font-bold transition hover:-translate-y-0.5 hover:shadow-sm ${i === 0 ? "bg-mission-navy text-white" : "border border-slate-200 bg-white text-slate-600 hover:border-mission-gold"}`}>{topic}</button>)}</div>
